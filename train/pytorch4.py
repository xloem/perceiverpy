...
# goal for pytorch4: train with the batch[es] that produce the greatest reduction in loss on new data
#    take e.g. 2 batches of items
#    make 2*log2(n) combinations of items based on paired checkerboard patterns: 11111111, 11110000, 11001100, 10101010
#    store result for each combination
#    sort combinations by result
#    iterate through sorted combinations, best first, counting how many of each item is encountered
#    include the first n items that are encountered log2(n) times: these are the n best-performing items
#
#            theory:
#                an item produces a property of improves-result
#                so, each group containing that item has that property
#                the item can be found be enumerating all combinations of groups, and seeing which items are in all groups with the property
todo use util.py to implement above paragraph

def data_combinations(len):
    # for now we only consider combinations of 2 groups of data.  the algorithm is extendable for larger groups of data.
    # returns which items are taken from group 0, and which from group 1
    batches = torch.tensor([0] * len + [1] * len)
    perms = torch.randperm(len*2)
    yield batches[perms[:len]]
    yield batches[perms[len:]]
